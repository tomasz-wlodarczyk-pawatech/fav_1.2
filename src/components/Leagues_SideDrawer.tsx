...
interface Leagues_SideDrawerProps {
  isOpen: boolean;
  onClose: () => void;
  onApply?: (selectedLeagues: string[], selectedCountries: string[]) => void;
}
...
export function Leagues_SideDrawer({ isOpen, onClose, onApply }: Leagues_SideDrawerProps) {
  ...
  const handleApply = () => {
    ...
    if (selectedLeagues.length > 0 && !selectedLeagues.includes('All Leagues')) {
    ...
    if (onApply) {
      onApply(selectedLeagues, selectedCountries);
    }
    onClose();
  };

  const handleClearFilters = () => {
    setSelectedLeagues(['All Leagues']);
    setSelectedCountries([]);
    setExpandedCountries([]);
    
    // Also clear the filters in the parent component by calling onApply with empty values
    if (onApply) {
      onApply([], []); // Clear both leagues and countries in parent state
    }
  };
    ...
    <>
      ...
      <div className={`fixed top-0 right-0 h-full w-full sm:w-96 md:w-80 lg:w-96 xl:w-[400px] bg-[#f4f5f0] z-50 transform transition-transform duration-300 ease-in-out ${
        ...
        <div className="flex flex-col relative size-full">
          {/* Scrollable Content */}
          <div className="box-border content-stretch flex flex-col gap-3 items-end justify-start pt-3 px-3 relative flex-1 overflow-y-auto">
            {/* Close Button */}
