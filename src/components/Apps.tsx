...
function AppCard({ id, title, tagline, icon, iconBg = "bg-gradient-to-br from-gray-100 to-gray-200", category, rating, reviews, size = 'medium', featured = false, isFavorite = false, onFavoriteToggle, url }: AppCardProps) {
  const cardSize = {
    small: 'w-40',
    medium: 'w-52',
    large: 'w-64'
  ...
  return (
    <div className={`${cardSize[size]} bg-white rounded-xl border border-gray-100 p-4 hover:bg-gray-50 transition-colors flex-shrink-0 ${featured ? 'ring-2 ring-[#9ce800] ring-opacity-50' : ''}`}>
      {/* Desktop layout (361px+) */}
      <div className="w-80:hidden">
        <div className="flex items-center justify-between mb-3">
          <div className="cursor-pointer">
            <div className={`h-12 w-12 rounded-xl ${iconBg} flex items-center justify-center hover:scale-105 transition-transform`}>
              {icon}
            </div>
          </div>
        ...
        <div className="cursor-pointer block">
          <h3 className="font-semibold text-sm text-gray-900 mb-1 line-clamp-2 hover:text-[#9ce800] transition-colors">{title}</h3>
      ...
      <div className="hidden w-80:block">
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="cursor-pointer">
            <div className={`h-10 w-10 rounded-xl ${iconBg} flex items-center justify-center flex-shrink-0 hover:scale-105 transition-transform`}>
              {icon}
            </div>
          </div>
...
function AppListItem({ id, title, tagline, icon, iconBg = "bg-gradient-to-br from-gray-100 to-gray-200", category, rating, reviews, isFavorite = false, onFavoriteToggle, url }: AppCardProps) {
  return (
      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
        <div className="flex flex-col gap-1">
          ...
          <div className="h-[76px] relative">
            ...
            <div className="absolute left-0 top-[7px] cursor-pointer">
              <div className={`${iconBg} rounded-[13px] size-[68px] flex items-center justify-center hover:scale-105 transition-transform`}>
                {icon}
              </div>
            </div>
            ...
            <div className="absolute flex flex-col h-[57px] justify-between left-[85px] top-1.5 right-0">
              {/* Title */}
              <div className="cursor-pointer">
                <h3 className="font-normal text-black leading-[28px] text-[14px] hover:text-[#9ce800] transition-colors">{title}</h3>
              </div>
...
function SectionHeader({ title, subtitle, showSeeAll = true, showGoBack = false, category, onSeeAllClick, onGoBackClick }: SectionHeaderProps) {
  ...
  const getCategoryCount = (categoryName: string) => {
    switch (categoryName) {
      case "Popular Apps": return 25;
      case "Betting Utilities": return 18;
      case "Social & Community": return 12;
      case "Content & Media": return 15;
      case "Partner Apps": return 8;
      case "Innovation": return 6;
      case "Special Events": return 4;
      default: return 0;
    }
  };
...
export function Apps({ activeItem, onAccountClick, onNavigationClick, onHelpClick, onFiltersClick, onBetslipClick, selectedLeagues, isLoggedIn, setIsLoggedIn, onAISearchClick, onSearchWithQuery }: AppsProps) {
  const navigate = useNavigate();
  
  // Helper function to safely read/write from localStorage
  const readFavoriteApps = (): string[] => {
    try {
  ...
  const [favoriteApps, setFavoriteApps] = useState<string[]>(readFavoriteApps);
  ...
  const popularApps = [
    ...
    },
    {
      ...
      title: "Mdundo Casino",
      tagline: "Play slots & more on Mdundo's vibing Casino!",
            ...
            <div className="w-full mb-8">
              ...
              {favoritedApps.length > 0 ? (
                <div className="grid grid-cols-2 gap-4 px-4">
                  {favoritedApps.slice(0, 4).map((app) => (
                    <div key={app.id} className="bg-white rounded-xl border border-gray-100 p-4 hover:bg-gray-50 transition-colors">
                      {/* App Icon and Star */}
                      <div className="flex items-center justify-between mb-3">
                        <div className="cursor-pointer">
                          <div className={`h-12 w-12 rounded-xl ${app.iconBg} flex items-center justify-center hover:scale-105 transition-transform`}>
                            {app.icon}
                          </div>
                        </div>
                        <button 
                      ...
                      {/* App Info and Open Button */}
                      <div className="cursor-pointer block">
                        <h3 className="font-semibold text-sm text-gray-900 mb-1 line-clamp-2 hover:text-[#9ce800] transition-colors">{app.title}</h3>
                        <div className="flex items-center justify-between mt-2">
                          <span className="text-xs text-gray-600 line-clamp-1">{app.category}</span>
                          <button className="bg-[#9ce800] text-black px-3 py-1 rounded-full text-xs font-medium hover:bg-[#8bd400] transition-colors">
                            OPEN
                          </button>
                        </div>
                ...
                <div className="mx-4 text-center py-8 bg-gray-50 rounded-xl border border-gray-200">
                  <div className="w-12 h-12 rounded-full bg-gray-100 mx-auto mb-3 flex items-center justify-center">
                    <Star className="h-6 w-6 text-gray-400" />
                  </div>
                  <p className="text-gray-600 mb-4">No favourite apps yet. Head to the App Store to add some.</p>
                  <button 
                    onClick={() => {
                      navigate('/apps');
                      window.scrollTo(0, 0);
                    }}
                    className="bg-[#9ce800] text-black px-4 py-2 rounded-full text-sm font-medium hover:bg-[#8bd400] transition-colors"
                  >
                    Open AppStore
                  </button>
                </div>
